...
require([
  ...
  var isViewerActivated = false; // 뷰어 활성화 여부
   ...
   * 아이콘도 함께 업데이트한다.
   */
  function setViewerActivated(isActivated) {
    isViewerActivated = isActivated;
  ...
  pubsub.sub('@-viewer-activated', function () {
    setViewerActivated(true);
  });

  pubsub.sub('@-viewer-inactivated', function () {
    setViewerActivated(false);
  });
